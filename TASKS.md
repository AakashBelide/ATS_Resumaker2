...
| 1.2 | **JD structuring POC** ⚡ — raw JD → structured: title, company, location, **work model** (onsite/hybrid/remote + state/timezone), seniority, required vs preferred quals, responsibilities, salary, **work-auth/sponsorship note**, **knockout questions**. | ⬜ Todo | 1.1 | — | Treat JD text as untrusted data (prompt-injection safe). |
| 1.3 | **Keyword extraction POC** ⚡ — triple-pass consensus (ATS-Resumaker style) → 15–20 weighted keywords, **hard vs soft** split. Persist a `standardized_keywords` set for deterministic reuse. | ⬜ Todo | 1.2 | — | Hard skills weighted higher (Jobscan method). |
...
| 1.5 | **Sponsorship POC** ⚡ — ingest DOL OFLC LCA + USCIS Hub subsets for a handful of test employers; **employer-name normalization**; sponsorship-likelihood (trailing-3-FY LCA volume + recency + SOC/title match + approval rate). | ⬜ Todo | 0.5 | — | Data-eng heavy = name normalization. Files download fine even though HTML 403s. Independent of 1.1–1.4. |
...
- **2026-08-07 — Task 1.1 ✅ (JD scraper).** `pocs/scrape_jd/` with public-ATS-API path (Greenhouse/Lever/Ashby) + Playwright fallback. Eval 4/4 on live postings. Owner added **Firecrawl** to the stealth shortlist (deferred sub-task with Scrapling/CloakBrowser). Next: 1.2 JD structuring (critical path) + 1.5 sponsorship (independent, can parallelize).
