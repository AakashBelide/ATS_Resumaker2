...
| # | Task | Status | Deps | Owner | Observations |
|---|------|--------|------|-------|--------------|
| 4.1 | **Stack decision** — FastAPI (recommended: Python ecosystem for docx/pdf/parsing) vs Node. | ✅ Done | Phase 2 | — | FastAPI chosen and shipped (`apps/api/`). Superseded by the R-series rebuild + deployment (D.0–D.9). |
| 4.2 | **Data model + Postgres** — files canonical, DB derived (history/analytics/monitoring, incl. token/cost/latency per ATS-Resumaker). | ✅ Done | 4.1 | — | Delivered as files-canonical + SQLite/Turso derived (`persistence/`), `companies`/`jobs`/`runs`/`tracker`. Postgres was not used; libSQL/Turso chosen at deploy (D.1). |
| 4.3 | **Pipeline-as-API + SSE** progress. | ✅ Done | 4.1, 2.1 | — | Pipeline-as-API live; SSE later replaced by status.json polling for serverless (D.6). |
| 4.4 | **Job ingestion + sponsorship enrichment services** (provider registry). | ✅ Done | 4.2, 1.5 | — | Ingestion + sponsorship live (`ingestion/`, `pocs/sponsorship`), provider/source registries in `providers/`. |
...
|------|----------------|--------|
| **Two GitHubs** (Personal `AakashBelide` vs Academic `Belide-Aakash`, latest work in Academic) | **DECIDED (2026-08-14): use the Academic GitHub (`Belide-Aakash`) for now** — it holds most of the recent/strongest projects. Resume's single code link points there. Revisit consolidation (mirror top projects into one handle) as later cleanup. | ✅ Decided |
...
| Scraping stealth tools legality | **CONFIRMED (2026-08-14): everything is official public APIs.** JD scraping (`providers/scrape/scraper.py`) uses each board's own public JSON API first — Greenhouse, Lever, Ashby, Workday CXS, Oracle CE, **Amazon `search.json`** (added 2026-08-14) — with a plain headless-Playwright fallback only for arbitrary pages, within ToS. Ingestion adapters (`providers/sources/`) are likewise official board/search APIs. `curl_cffi` Chrome impersonation is used only for TLS/JA3-picky **public .gov** endpoints (USCIS sponsorship data). No stealth/cloaking tools (Scrapling/CloakBrowser) are in use. | ✅ Decided |
...
| R1 | **Skeleton + packaging** — monorepo dirs, `pyproject.toml` (single package + extras), tooling (ruff, mypy, pytest, Makefile/justfile). | ✅ Done | Shipped: `src/resumaker/` package, `pyproject.toml` extras, ruff/mypy/pytest via `Makefile`. `uv sync` clean. |
| R2 | **Core foundation** — `config/` (pydantic-settings), `domain/` schemas, `observability/` (structured logging, `/metrics` counters, cost guard), `persistence/` (file store canonical + SQLite derived + cache store). **DB schema includes `companies`/`jobs`/`runs` for ingestion from day one.** | ✅ Done | Live: `config/settings.py`, `domain/`, `observability/`, `persistence/` (file + SQLite/Turso). `companies`/`jobs`/`runs`/`tracker` present. |
| R3 | **Provider layer** — LLM `registry` (claude_cli · anthropic_api · gemini) behind one `LLMProvider` interface + **prompt-hash response cache**; scrape registry; `sources/` board-listing seam. | ✅ Done | Live: `providers/llm/registry.py` (claude_cli/anthropic/gemini + FallbackProvider), scrape + `sources/` registries. |
| R4 | **Stages + pipeline** — migrate every `pocs/*` into `stages/*` behind a uniform `Stage` interface; port orchestrator to a stage DAG + `ProgressReporter`. **Parity gate:** a full run matches POC output (fact-gate PASS, ATS-verify PASS, 1-page, grounded). | ✅ Done | Live: `stages/*` + `run_pipeline`; parity verified (fact-gate/ATS/1-page/grounded) on live JDs. |
| R5 | **API service** — FastAPI app factory; routers: `health`, `runs` (start/get/list), `jobs` (watchlist), `sse` (progress stream reusing the emitter), `profile`, `costs`; in-process worker + persisted job queue; **token auth** + rate limit; no PII in logs. | ✅ Done | Live: `apps/api/` routers (health/runs/tracker/onboard/profile/costs/worker); token auth; SSE→polling (D.6). |
| R6 | **CLI** — Typer app over the library: `run`, `watch`, `costs`, `ingest`, `serve`. Back-compat with today's commands. | ✅ Done | Live: `apps/cli/main.py`. |
| R7 | **Web + extension scaffolds** — Next.js dashboard skeleton (review/approve/download, history, cost/quality panels) + MV3 extension skeleton (capture JD → call API). Not feature-complete; wired to the API contract. | ✅ Done | Superseded by the full Phase-5 build: `web/` (all pages live) + thin MV3 `extension/`. |
| R8 | **Deploy** — multi-stage slim Dockerfile, `docker-compose.yml` (api + Caddy auto-HTTPS; optional Grafana), `.env.example`, systemd unit. Deploy-agnostic. | ✅ Done | Superseded/expanded by D.0–D.9: split Dockerfiles, `docker-compose.split.yml`, Terraform, live on Cloud Run. |
| R9 | **Cutover** — full test suite green, parity confirmed on a 3-JD regression; delete `resumaker/`; update README/docs; final commit. | ✅ Done | Everything runs from `src/resumaker/`; tests green; README/docs current. POC tree kept under `pocs/` intentionally as reference. |
...
- **Failed matches are visible + retryable.** Added `tracker.match_error` (DB col + additive migration + `TrackerEntry` field). `_apply_match` records the error (and clears it on success) instead of leaving `fit=NULL`, which the UI could not distinguish from "in progress" → eternal `matching…` + infinite polling. New `POST /v1/tracker/{id}/rematch`; Tracker page shows a `failed · retry` state and stops polling failed rows. Test: `test_tracker_match_failure_sets_error_then_retry_clears`. **Durability (updated 2026-08-14):** in the **deployed** environment matches are durable/retryable — the api runs with `RESUMAKER_JOB_QUEUE=cloud_tasks` (`run.tf`), so `submit_tracker_match` enqueues a Cloud Task to the worker's `/v1/worker/tracker-match`, which runs the match **inline** (Tasks awaits + retries on non-2xx; queue `retry_config max_attempts=3`, backoff 10s–300s). `run_match_for` records/clears `match_error`, so retries are idempotent. Only **local** dev uses the in-process ThreadPool (`InProcessQueue`), which has no cross-process durability — expected for local.
