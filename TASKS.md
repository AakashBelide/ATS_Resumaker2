...
| 1.11 | **Deterministic ATS scorer POC** ⚡ — transparent weighted keyword/skill overlap (50/30/20) + **semantic per-requirement cosine coverage (blueprint §11 semantic/AI-ATS self-check)**. Reproducible metric. | ⬜ Todo | 1.3, 1.8 | — | Honest proxy, not a "real ATS score." §11's *writing* side (skill+action+result+context bullets, recency) already live in tailoring; the *mechanical* embed-per-requirement cosine coverage is delivered HERE. |
...
| 1.13 | **Enrichment & preferences memory** (career-ops `_profile.md`/`_custom.md` parallel) — persistent layer the system reads every run + updates from conversation. | ⬜ Todo | 0.3 | — | (a) `preferences` (target roles/archetypes, comp, location, work-model, sponsorship need, seniority) → feeds role-fit/apply/tailoring; (b) `house_rules` (learned corrections applied every run, e.g. no em-dashes, combine Bajaj, keep projects); (c) `enrichment log` + an update step that folds new facts/corrections from conversation into the canonical `profile.json` and records "do-not-repeat" items. |
