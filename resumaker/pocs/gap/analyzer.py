...
from core.schemas import GapItem, GapReport, JobPosting, KeywordSet
...
if __name__ == "__main__":
    ...
    report = analyze_gaps(structure_jd(scrape(sys.argv[1])))
    for it in report.items:
        tag = it.status if not it.substitution else f"{it.status}~>{it.substitution}"
        print(f"  [{tag:22}] {it.requirement[:70]}")
    print("\nTRUE GAPS:", report.gaps)
    print("BRIDGES:", report.substitutions)
