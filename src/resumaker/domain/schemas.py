"""Shared I/O contracts across all pipeline stages. Every stage produces/consumes
...
class JobPosting(BaseModel):
    """Structured JD (structure stage output)."""
    title: str = ""
    company: str = ""
    location: str = ""
    ...
    # authoritative, role-specific signal; overrides USCIS history in the decision):
...
class KeywordSet(BaseModel):
    """Keyword stage output. `standardized` is the frozen set reused for scoring."""
...
class FitScore(BaseModel):
    """Role-fit stage output. Deterministic + LLM-anchored, per blueprint §12/§13."""
...
class ResumeContent(BaseModel):
    """Tailored, still-structured resume before rendering."""
    headline: str = ""
    summary: str = ""
...
class VerifyReport(BaseModel):
    """Fact-gate / ATS-verify output. `passed` gates the pipeline."""
    passed: bool = True
...
class CoverLetter(BaseModel):
    """Cover-letter output. Grounded, anti-AI-tell, human-reviews-before-send."""
    company: str = ""
    role: str = ""
...
class ATSScore(BaseModel):
    """Transparent keyword/skill-overlap proxy (NOT a real ATS score).
    overall = 0.5*keyword + 0.3*quantification + 0.2*structure."""
    ...
    semantic_coverage: float = 0.0       # 0-100 = % of JD requirements evidenced
...
class PipelineResult(BaseModel):
    """Orchestrator output: every stage's result in one object. Defined last so all
    referenced stage models already exist."""
    url: str = ""
    out_dir: str = ""
    ...
    sponsorship: dict[str, Any] = Field(default_factory=dict)   # SponsorshipVerdict fields
