...
def add(*, job_id: int | None = None, url: str | None = None,
    ...
    """Add a job to the tracker and (by default) run its match pipeline. Provide either a
    watchlist `job_id` (from Discovery) or a raw `url`. Re-adding the same url refreshes the
    match fields but preserves the owner's `stage`/`notes` (see db.upsert_tracker)."""
    company = title = ""
    ...
    if run_match:
        ...
        res = run_pipeline(url=target_url, match_only=True)
        if res.error:
            _log.warning("tracker match failed", extra={"url": target_url, "error": res.error})
        if res.job is not None:                       # prefer the structured JD's fields
            entry.company = res.job.company or company
            entry.title = res.job.title or title
