...
5. "skills": be COMPREHENSIVE - include ALL of the candidate's skills that match or relate to the JD (existing + supported), in the JD's wording. Do NOT minimize or collapse categories to 3-4 items each: aim ~28-32 total items across 5-6 JD-aligned categories. If the candidate has them and they are relevant to the role, you MUST include the DevOps/infra/deployment stack - **Docker, Kubernetes, Terraform, CI/CD** - and the data/orchestration stack - **Snowflake, BigQuery, Airflow, PySpark/Spark, Databricks** - and GenAI - **Prompt Engineering, RAG, Multi-Agent Orchestration, Azure OpenAI** - whenever the role is an AI/ML/engineering role (these are standard, recruiter-searched tools even if the JD does not spell each one out; do NOT drop a grounded, role-standard tool just because the JD text omits the exact word). ALSO include the candidate's grounded DOMAIN tools that are CENTRAL to THIS role even if the JD never names the exact tool - judge by the role's core theme: e.g. for a graph / identity-graph / entity-resolution / record-linkage role, their graph databases (**Neo4j, TigerGraph**) and dedup/record-linkage skills; for a search role, their search/vector stack (Elasticsearch, vector DBs). A skill the candidate demonstrably has that is core to the role MUST appear - never omit it just because the JD text or target keywords omit the tool name. Cut only true duplicates (e.g. "Enterprise RAG" + "RAG") and clearly off-role skills. You may add an equivalence bridge (format: "GCP Cloud Run (AWS Lambda-equivalent)"). Never add a skill the candidate does not have.
